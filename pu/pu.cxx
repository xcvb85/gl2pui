...
int puShaderProgramSingleColor;
int puShaderProgramMultipleColors;

static const GLchar *vShaderSingleColor =
        "#version 100\n\n"
        ...
        "   gl_Position = vec4(vPosition.x * 2.0 / screenSize.x - 1.0, 1.0 - (screenSize.y - vPosition.y) * 2.0 / screenSize.y, 0.0, 1.0);\n"
        "}\n";

static const GLchar *fShaderSingleColor =
        "#version 100\n\n"
    #ifndef __APPLE__
        "precision mediump float;\n"
    #endif
        "uniform vec4 vColor;\n"
        "uniform float vStripple;\n"
        "varying vec2 VertexPosition;\n"
        "void main()\n"
        "{\n"
        "    if(vStripple > 0.0) {\n"
        "        if(mod(VertexPosition.x+VertexPosition.y, vStripple) > 0.5*vStripple) {\n"
        "            discard;\n"
        "        }\n"
        "    }\n"
        "    gl_FragColor = vColor;\n"
...
static const GLchar *vShaderMultipleColors =
        "#version 100\n\n"
        "attribute vec2 vPosition;\n"
...
static const GLchar *fShaderMultipleColors =
        "#version 100\n\n"
    #ifndef __APPLE__
        "precision mediump float;\n"
    #endif
...
void puShaderInit ( void )
{
    ...
    puVertShader = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(puVertShader, 1, &vShaderSingleColor, 0);
    glCompileShader(puVertShader);

    puFragShader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(puFragShader, 1, &fShaderSingleColor, 0);
    glCompileShader(puFragShader);

    puShaderProgramSingleColor = glCreateProgram();
    glAttachShader(puShaderProgramSingleColor, puVertShader);
    glAttachShader(puShaderProgramSingleColor, puFragShader);
    glBindAttribLocation(puShaderProgramSingleColor, 0, "vPosition");
    glLinkProgram(puShaderProgramSingleColor);


    puVertShader = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(puVertShader, 1, &vShaderMultipleColors, 0);
    glCompileShader(puVertShader);

    puFragShader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(puFragShader, 1, &fShaderMultipleColors, 0);
    glCompileShader(puFragShader);

    puShaderProgramMultipleColors = glCreateProgram();
    glAttachShader(puShaderProgramMultipleColors, puVertShader);
    glAttachShader(puShaderProgramMultipleColors, puFragShader);
    glBindAttribLocation(puShaderProgramMultipleColors, 0, "vPosition");
    glBindAttribLocation(puShaderProgramMultipleColors, 1, "vColor");
    glLinkProgram(puShaderProgramMultipleColors);
}
